        ...
        $("#system_jqx_container").jqxGrid(
            {
                ...
                columns: [
                    ...
                    { text: '<?php echo $CI->lang->line('LABEL_VARIETY_NAME'); ?>',pinned:true, dataField: 'variety_name',width:'150',cellsrenderer: cellsrenderer,align:'center',editable:false},
                        <?php
                            foreach($areas as $area)
                            {?>{ columngroup: 'area',text: '<?php echo $area['text'];?>', dataField: '<?php echo 'area_quantity_'.$area['value'];?>',align:'center',width:'90',cellsrenderer: cellsrenderer,cellsAlign:'right',editable:false},
                    <?php
                        }
                    ?>
                        <?php
                            for($i=0;$i<=$CI->config->item('num_year_prediction');$i++)
                            {?>{ columngroup: '<?php echo 'year'.$i.'_id'; ?>',text: 'ZI', dataField: '<?php echo 'year'.$i.'_area_total_quantity';?>',align:'center',width:'80',cellsrenderer: cellsrenderer,cellsAlign:'right',editable:false},
                    {
                        columngroup: '<?php echo 'year'.$i.'_id'; ?>',text: 'DI', dataField: '<?php echo 'year'.$i.'_budget_quantity';?>',align:'center',width:'110',cellsrenderer: cellsrenderer,cellsAlign:'right',columntype:'custom',
                ...
                columngroups:
                    [
                        { text: 'Zones', align: 'center', name: 'area' },
