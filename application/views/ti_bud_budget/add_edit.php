        ...
        $("#system_jqx_container").jqxGrid(
            {
                ...
                columns: [
                    ...
                    <?php
                        foreach($customers as $customer)
                        {?>{ columngroup: 'customers',text: '<?php echo $customer['text'];?>', dataField: '<?php echo 'customer_quantity_'.$customer['value'];?>',align:'center',width:'200',cellsrenderer: cellsrenderer,cellsAlign:'right',editable:false},
                    <?php
                        }
                    ?>
                        <?php
                            for($i=0;$i<=$CI->config->item('num_year_prediction');$i++)
                            {?>{ columngroup: '<?php echo 'year'.$i.'_id'; ?>',text: '<?php if($i>0){echo "Customer Prediction";}else{echo "Customer Budget";} ?>', dataField: '<?php echo 'year'.$i.'_customer_total_quantity';?>',align:'center',width:'150',cellsrenderer: cellsrenderer,cellsAlign:'right',editable:false},
                    {
