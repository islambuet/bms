...
<div class="row widget">
    ...
    if(isset($CI->permissions['column_headers'])&&($CI->permissions['column_headers']==1))
    {
        ...
        <div class="col-xs-12" style="margin-bottom: 20px;">
            ...
            <label class="checkbox-inline"><input type="checkbox" class="system_jqx_column"  checked value="tp_final">Final TP</label>
            <label class="checkbox-inline"><input type="checkbox" class="system_jqx_column"  checked value="commission_final">Commission %</label>
            <label class="checkbox-inline"><input type="checkbox" class="system_jqx_column"  checked value="sales_commission">Commission</label>
            <label class="checkbox-inline"><input type="checkbox" class="system_jqx_column"  checked value="incentive_final">Incentive %</label>
...
<script type="text/javascript">
    function calculate_total(row, tp_final, sales_commission_percentage,incentive_percentage)
    {
        ...
        var sales_commission=tp_final*sales_commission_percentage/100;
        var incentive=tp_final*incentive_percentage/100;
        ...
        var net_price=tp_final-sales_commission-incentive;
        if(net_price!=0)
        {
        ...
        {
            $("#system_jqx_container").jqxGrid('setcellvalue', row, 'profit_percentage', '0');
    ...
    $(document).ready(function ()
    {
        var cellsrenderer = function(row, column, value, defaultHtml, columnSettings, record)
        {
            ...
            if(column=='tp_final' ||column=='commission_final' ||column=='incentive_final')
            {
        ...
        var source =
        {
            ...
            dataFields: [
                ...
                { name: 'tp_final', type: 'string' },
                { name: 'commission_final', type: 'string' },
                { name: 'sales_commission', type: 'string' },
                { name: 'incentive_final', type: 'string' },
        ...
        $("#system_jqx_container").jqxGrid(
            {
                ...
                columns: [
                    ...
                    {text: 'Hom TP', dataField: 'tp_hom',align:'center',cellsalign: 'right',width:'110',cellsrenderer: cellsrenderer,rendered: tooltiprenderer,editable:false},
                    {text: 'Final TP', dataField: 'tp_final',align:'center',cellsalign: 'right',width:'150',cellsrenderer: cellsrenderer,rendered: tooltiprenderer,editable:true,columntype:'custom',
                        ...
                        cellendedit: function (row, column, editor,oldvalue,value)
                        {
                            ...
                            if(!isNaN(parseFloat(row_data['commission_final'])))
                            {
                                sales_commission_percentage=parseFloat(row_data['commission_final']);
                            }
                            var incentive_percentage=0;
                            if(!isNaN(parseFloat(row_data['incentive_final'])))
                            {
                                incentive_percentage=parseFloat(row_data['incentive_final']);
                            }
                            calculate_total(row, value, sales_commission_percentage,incentive_percentage);
                        }
                    },
                    {text: 'Commission %', dataField: 'commission_final',align:'center',cellsalign: 'right',width:'110',cellsrenderer: cellsrenderer,rendered: tooltiprenderer,editable:true,columntype:'custom',
                        initeditor: function (row, cellvalue, editor, celltext, pressedkey) {
                            editor.html('<div style="margin: 0px;width: 100%;height: 100%;padding: 5px;"><input type="text" value="'+cellvalue+'" class="jqxgrid_input float_type_positive"><div>');
                        },
                        geteditorvalue: function (row, cellvalue, editor) {
                        ...
                        cellendedit: function (row, column, editor,oldvalue,value)
                        {
                            var row_data = $('#system_jqx_container').jqxGrid('getrowdata', row);
                            var tp_final=0;
                            if(!isNaN(parseFloat(row_data['tp_final'])))
                            {
                                tp_final=parseFloat(row_data['tp_final']);
                            }
                            var incentive_percentage=0;
                            if(!isNaN(parseFloat(row_data['commission_final'])))
                            {
                                incentive_percentage=parseFloat(row_data['commission_final']);
                            }
                            calculate_total(row, tp_final,value,incentive_percentage);
                        }
                    },
                    ...
                    {text: 'Incentive %', dataField: 'incentive_final',align:'center',cellsalign: 'right',width:'110',cellsrenderer: cellsrenderer,rendered: tooltiprenderer,editable:true,columntype:'custom',
                        initeditor: function (row, cellvalue, editor, celltext, pressedkey) {
                            editor.html('<div style="margin: 0px;width: 100%;height: 100%;padding: 5px;"><input type="text" value="'+cellvalue+'" class="jqxgrid_input float_type_positive"><div>');
                        },
                        geteditorvalue: function (row, cellvalue, editor) {
                        ...
                        cellendedit: function (row, column, editor,oldvalue,value)
                        {
                            ...
                            calculate_total(row, tp_final, sales_commission_percentage,value);
                        }
                    },
