...
class Hom_bud_quantity_finalize extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Hom_bud_quantity_finalize');
        ...
        $this->controller_url='hom_bud_quantity_finalize';
    ...
    private function system_search()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $data['title']="Quantity Finalize";
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("hom_bud_quantity_finalize/search",$data,true));
            if($this->message)
            {
    ...
    private function system_list()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view("hom_bud_quantity_finalize/list",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function get_items($year0_id)
    {
        ...
        $this->db->select('fhom.status_quantity_finalize');
        ...
        {
            if(!$item['status_quantity_finalize'])
            {
                $item['status_quantity_finalize']=$this->config->item('system_status_no');
            }
        }
    ...
    private function system_edit($year0_id,$crop_id)
    {
        if((isset($this->permissions['edit'])&&($this->permissions['edit']==1))||(isset($this->permissions['add'])&&($this->permissions['add']==1)))
        {
            ...
            if((!isset($this->permissions['edit'])||($this->permissions['edit']!=1)))
            {
                ...
                if($info)
                {
                    if($info['status_quantity_finalize']===$this->config->item('system_status_yes'))
                    {
                        $ajax['status']=false;
            ...
            $data['title']="Quantity Finalize For ".$crop['text'].'('.$data['years'][0]['text'].')';
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("hom_bud_quantity_finalize/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
        ...
        foreach($results as $index=>$result)
        {
            ...
            if($index>0)
            {
                if($prev_type!=$result['type_name'])
                {
                    ...
                    $total_item['variance']='';
                    $total_item['final_quantity_editable']=false;
                    $items[]=$total_item;
                    $total_types=0;
                    $count=0;
            ...
            $item['quantity_needed']=$item['year0_budget_quantity']-$item['variance'];
            if($item['quantity_needed']<0)
            {
                $item['quantity_needed']='-';
            }
            $item['final_quantity']='';//check already saved get it
            $item['final_quantity_editable']=true;//check already saved get it
