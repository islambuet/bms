...
class Hom_bud_variance_finalize extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Hom_bud_variance_finalize');
    ...
    private function system_search()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $data['title']="Variance Finalize";
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("hom_bud_variance_finalize/search",$data,true));
            if($this->message)
            {
    ...
    private function system_list()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view("hom_bud_variance_finalize/list",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function get_items($year0_id)
    {
        ...
        $this->db->select('fhom.status_variance_finalize');
        ...
        {
            if(!$item['status_variance_finalize'])
            {
                $item['status_variance_finalize']=$this->config->item('system_status_no');
            }
        }
    ...
    private function system_edit($year0_id,$crop_id)
    {
        if((isset($this->permissions['edit'])&&($this->permissions['edit']==1))||(isset($this->permissions['add'])&&($this->permissions['add']==1)))
        {
            ...
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("hom_bud_variance_finalize/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
