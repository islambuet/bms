...
class Zi_bud_ti_target extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Zi_bud_ti_target');
        ...
        $this->controller_url='zi_bud_ti_target';
    ...
    private function system_search()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $data['title']="Assign TI Target";
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("zi_bud_ti_target/search",$data,true));
            if($this->message)
            {
    ...
    private function system_list()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view("zi_bud_ti_target/list",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function system_edit($zone_id,$year0_id,$crop_id)
    {
        ...
        if((isset($this->permissions['edit'])&&($this->permissions['edit']==1))||(isset($this->permissions['add'])&&($this->permissions['add']==1)))
        {
            ...
            $data['title']="Assign TI Target For ".$crop['text'].'('.$data['years'][0]['text'].')';
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("zi_bud_ti_target/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
                ...
                foreach($items as $territory_id=>$item)
                {
                    foreach($item as $variety_id=> $year0_target_quantity)
                    {
                        ...
                        if(isset($area_budget_target[$territory_id][$variety_id]))
                        {
                            ...
                            if($year0_target_quantity!=$area_budget_target[$territory_id][$variety_id]['year0_target_quantity'])
                            {
    ...
    private function system_details($zone_id,$year0_id,$crop_id)
    {
        ...
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $data['title']="TI Target For ".$crop['text'].'('.$data['years'][0]['text'].')';
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("zi_bud_ti_target/details",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
