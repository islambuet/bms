...
class Ti_bud_customer_target_prediction extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Ti_bud_customer_target_prediction');
        ...
        $this->controller_url='ti_bud_customer_target_prediction';
    ...
    private function system_search()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("ti_bud_customer_target_prediction/search",$data,true));
            if($this->message)
            {
    ...
    private function system_list()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view("ti_bud_customer_target_prediction/list",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function get_items($territory_id,$year0_id)
    {
        ...
        $this->db->select('fzi.status_prediction_assign status_prediction_finalize');
        $this->db->select('fti.status_prediction_assign status_prediction_assign');
    ...
    private function system_edit($territory_id,$year0_id,$crop_id)
    {
        ...
        if((isset($this->permissions['edit'])&&($this->permissions['edit']==1))||(isset($this->permissions['add'])&&($this->permissions['add']==1)))
        {
            ...
            $data['title']="Assign Customers 3years target For ".$crop['text'].'('.$data['years'][0]['text'].')';
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("ti_bud_customer_target_prediction/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function system_details($territory_id,$year0_id,$crop_id)
    {
        ...
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $data['title']="Assign Customers 3years target For ".$crop['text'].'('.$data['years'][0]['text'].')';
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("ti_bud_customer_target_prediction/details",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
