...
class Setup_currency extends Root_Controller
{
    ...
    private function system_list()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_currency/list",$data,true));
            if($this->message)
            {
    ...
    {
        $this->db->from($this->config->item('table_setup_currency'));
    ...
    private function system_add()
    {
        if(isset($this->permissions['add'])&&($this->permissions['add']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_currency/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['edit'])&&($this->permissions['edit']==1))
        {
            ...
            $data['currency']=Query_helper::get_info($this->config->item('table_setup_currency'),'*',array('id ='.$currency_id),1);
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            if($id>0)
            {
                ...
                Query_helper::update($this->config->item('table_setup_currency'),$data,array("id = ".$id));
            ...
            else
            {
                ...
                Query_helper::add($this->config->item('table_setup_currency'),$data);
            }
