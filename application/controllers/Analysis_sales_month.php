...
class Analysis_sales_month extends Root_Controller
{
    ...
    private function get_items_year()
    {
        $items=array();
        $year0_id=$this->input->post('year0_id');
        $years=Query_helper::get_info($this->config->item('ems_basic_setup_fiscal_year'),array('id value','name text','date_start','date_end'),array('id <='.$year0_id,'status ="'.$this->config->item('system_status_active').'"'),5,0,array('id DESC'));
        ...
        $this->db->where('pod.revision',1);
        $this->db->where('po.status_approved',$this->config->item('system_status_po_approval_approved'));
        $this->db->where('po.date_approved >=',$years[sizeof($years)-1]['date_start']);
        $this->db->where('po.date_approved <=',$years[0]['date_end']);
        if($crop_id>0)
        {
            $this->db->where('type.crop_id',$crop_id);
            if($crop_type_id>0)
        ...
        foreach($results as $result)
        {
            ...
            if(in_array($m,$months))
            {
                ...
                {
                    if(($result['date_approved']>=$year['date_start'])&&($result['date_approved']<=$year['date_end']))
                    {
                        $year_id=$year['value'];
                    }
                }
